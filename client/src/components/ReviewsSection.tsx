    ...
    <section id="reviews" className="relative py-24 md:py-32">
      <div ref={ref} className="container">
        ...
        <motion.div
          ...
          className="flex flex-col md:flex-row items-center justify-center gap-10 mb-16 p-8 rounded-2xl bg-[#f7e7ce]/20 border border-border/20 max-w-3xl mx-auto"
        >
          {/* Big Rating */}
          ...
          <div className="flex-1 w-full max-w-xs space-y-2">
            {ratingDistribution.map((item) => (
              <div key={item.stars} className="flex items-center gap-3">
                <span
                  className="text-foreground text-sm w-3 text-right"
                  style={{ fontFamily: "'Outfit', sans-serif" }}
                >
                  {item.stars}
                </span>
                ...
                <span
                  className="text-foreground text-xs w-8"
                  style={{ fontFamily: "'Outfit', sans-serif" }}
                >
                  {item.percentage}%
                </span>
        ...
        >
          {[
            ...
            <span
              key={tag.label}
              className="px-4 py-2 rounded-full bg-[#f7e7ce]/20 border border-border/20 text-foreground text-sm hover:border-gold/30 hover:text-gold transition-all duration-300"
              style={{ fontFamily: "'Outfit', sans-serif" }}
            >
              {tag.label} <span className="text-foreground">{tag.count}</span>
            </span>
          ))}
        </motion.div>
      </div>
